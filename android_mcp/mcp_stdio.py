...
class StdioMcpServer:
    ...
    call returns an error to Cursor instead of sitting until the client's ~300s
    abort. Parallel CallMcpTool from the client still queues; prefer sequential
    health checks over fan-out.
    ...
    * If a handler ignores the deadline (pure-Python spin, un-killable native
    ...
    def _call_tool_guarded(self, params: JsonDict) -> JsonDict:
        ...
        Cursor often fans out multiple CallMcpTool in one turn. This server can
        only process them sequentially over stdio; without a hard timeout a single
        stuck adb/frida handler makes the whole batch appear hung until Cursor's
        ~300s client abort.
        """
        ...
        try:
            ...
            # wedge every subsequent tool (the previous 'reaper' design did exactly
            # that whenever a handler blocked past the wall timeout).
