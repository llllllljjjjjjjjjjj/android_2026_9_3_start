# IDA 9.2: dump 0x31 区域 zstd 核心函数（解压 + 字典加载定位）
import ida_auto, ida_pro, ida_funcs, idc, idautils, ida_idaapi
import json, time

OUT = r"D:\reserve_agent\ish-portable-kit\projects\dy\artifacts\zstd_core_funcs.json"
FUNCS = [0x3195b8, 0x3196cc, 0x319798, 0x31a004, 0x31a074, 0x31a098, 0x31a400, 0x318ff4, 0x31900c]
...
def main():
    ...
    for f in FUNCS:
        ...
        print(hex(f), "->", rec.get("addr"), "~", rec.get("end"), "size", rec.get("size"),
              "| strings:", list(rec.get("strings", {}).values())[:8])
