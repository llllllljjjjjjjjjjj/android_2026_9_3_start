...
$rows = foreach($d in $dirs){
    ...
    $samples = @(Get-ChildItem -LiteralPath $d.FullName -Force -ErrorAction SilentlyContinue | Select-Object -First 6 | ForEach-Object { $_.Name })
    [pscustomobject]@{ Name=$d.Name; SizeMB=$mb; Samples=($samples -join ' | ') }
}
$rows | Sort-Object SizeMB -Descending | Format-Table Name,SizeMB,Samples -Wrap -AutoSize | Out-String -Width 230
$rows | Export-Csv -LiteralPath 'D:\reserve_agent\android\_archive\tmp_c_drive_scan\roaming_sizes.csv' -NoTypeInformation -Encoding UTF8
Write-Output 'DONE'
