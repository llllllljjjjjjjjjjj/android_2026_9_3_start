...
class EnhancedDexDumpRunner:
    ...
    def __init__(self, verbose: bool = False, max_attempts: int = 3, deep_search: bool = False, 
        ...
        self.dexdump_path = None  # frida-dexdump 可执行路径

    def _find_frida_dexdump(self) -> Optional[str]:
        """查找 frida-dexdump 可执行文件（支持 PATH 和 venv Scripts）"""
        # 1. 先查 PATH
        path = shutil.which("frida-dexdump")
        if path:
            return path
        # 2. 查当前 Python venv 的 Scripts 目录（Claude bash 常见场景）
        ...
        for name in ("frida-dexdump.exe", "frida-dexdump"):
            ...
            if os.path.isfile(candidate):
                return candidate
        # 3. 查相邻的 .venv-frida-* Scripts 目录
        project_root = os.path.dirname(os.path.dirname(sys.prefix))  # 假设 venv 在项目根
        for pattern in (".venv-frida-16.5.7", ".venv-frida-16.7.19"):
            for name in ("frida-dexdump.exe", "frida-dexdump"):
                candidate = os.path.join(project_root, pattern, "Scripts", name)
                if os.path.isfile(candidate):
                    return candidate
        return None
    ...
    def check_environment(self) -> bool:
        ...
        # 检查frida-dexdump
        try:
            self.dexdump_path = self._find_frida_dexdump()
            if not self.dexdump_path:
                self.log("frida_dexdump_not_found", "ERROR")
                self.log("install_frida_dexdump", "WARNING")
                return False
            self.log("frida_dexdump_found", path=self.dexdump_path)
    ...
    def setup_frida_server(self) -> bool:
        ...
        try:
            # 1. 重命名 frida-server 伪装（历史成功策略；本环境主力 florida-server）
            original_server = "/data/local/tmp/florida-server"
    ...
    def execute_frida_dexdump(self, package_name: str, output_dir: str = None) -> bool:
        ...
        dexdump_cmd = self.dexdump_path or "frida-dexdump"
        cmd = [dexdump_cmd, "-U", "-p", str(pid)]
