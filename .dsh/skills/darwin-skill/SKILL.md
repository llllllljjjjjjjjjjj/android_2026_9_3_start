...
```bash
grep -nE "(在 Claude Code|Claude Code skill|Claude Code 用户|Cursor only|Codex 中|^\[!\[Claude Code|~/\.dsh/skills/[a-z]|/plugin install\b)" SKILL.md README.md 2>/dev/null
```
...
```
1. 确认优化范围：
   - 全部skills → 扫描 .dsh/skills/*/SKILL.md
   - 指定skills → 用户指定列表
...
文件位置：`.dsh/skills/darwin-skill/results.tsv`
...
8. **Runtime 中立性** — skill 必须能在 Claude Code、Codex、Cursor、OpenClaw、Hermes 等任何 skills-compatible runtime 中正常运行。除非 skill 名明确绑定单一 runtime（如 `xxx-codex`、`huashu-slides-codex`），任何「在 Claude Code 里」「Claude Code skill」「单一 badge 钉死」「安装命令只给 `.dsh/skills/` 一种路径」都视为 gate 不通过，须在 P0 优先修复（详见「Runtime 适配性审查」章节）
...
4. 用 scripts/screenshot.mjs 截图（2x 高清，只截 .card 元素，自动 open 图片）：
   node .dsh/skills/darwin-skill/scripts/screenshot.mjs \
